...
import {
  ...
  apiApply, apiMyApplications, apiJobApplicants, apiSubmitInterview,
} from "./api";

// ─── Question generation (still client-side until Phase 2) ───────────────────
...
function mockAnalyze() {
  const scores = {
    relevance:     Math.round(55 + Math.random() * 35),
    confidence:    Math.round(50 + Math.random() * 40),
    emotion:       Math.round(60 + Math.random() * 30),
    communication: Math.round(55 + Math.random() * 35),
  };
  scores.overall = Math.round(
    scores.relevance * 0.35 + scores.confidence * 0.25 +
    scores.emotion * 0.2  + scores.communication * 0.2
  );
  return scores;
}
  ...
  const handle = async () => {
    ...
    try {
      ...
      if (mode === "login") {
        ...
        data = await apiRegister({ name: form.name, email: form.email, password: form.password, role: form.role });
      }
    ...
    <div style={S.card}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
        <div>
          ...
          <div style={{ ...S.muted, fontSize:12 }}>{job.hr_name} · {job.department}</div>
        </div>
...
function JobForm({ initial, onSaved, onCancel }) {
  ...
  const save = async () => {
    ...
    try {
      if (initial) {
        await apiUpdateJob(initial.id, payload);
      } else {
...
// ─── Applicants View (HR) ─────────────────────────────────────────────────────
function ApplicantsView({ job, onBack }) {
...
// ─── Candidate Dashboard ──────────────────────────────────────────────────────
function CandidateDashboard({ user, onInterview }) {
  const [jobs,         setJobs]         = useState([]);
              ...
              return (
                <div key={job.id} style={S.card}>
                  ...
                  {!done ? (
                    ...
                    <div style={{ fontSize:13, color:"#10B981", fontWeight:500 }}>Interview completed ✓</div>
                  )}
                </div>
              );
            })}
...
function InterviewView({ job, application, onComplete, onCancel, stream }) {
  ...
  // "hidden" = question not yet revealed, "revealed" = shown + timer running
  ...
  // isAway: true while the candidate is already "away" — prevents double-counting
  const isAwayRef       = useRef(false);
  const warnTimeRef     = useRef(null);
  // Always holds the latest submitAnswer so recordViolation never captures a stale copy.
  const submitAnswerRef = useRef(null);
  // Ref to latest violations count (for use inside event-listener closures)
  const violationsRef   = useRef(0);

  // ── Restore checkpoint on mount (handles refresh / disconnect) ─────────────
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (cp.stage === "question" && cp.answers?.length > 0) {
          ...
          setStage("question");
          // Keep qState hidden so they must re-click Show Question
        }
      }
    } catch (_) {}
  ...
  // ── Checkpoint: save progress to localStorage after every answer ─────────────
  // Called inside submitAnswer. Clears on finish/disqualify.
  ...
  // ── Camera ──────────────────────────────────────────────────────────────────
  // The stream is owned and managed entirely by the parent App component.
  // InterviewView just attaches it to the <video> element when available.
  useEffect(() => {
  ...
  // ── Anti-cheat: all detection in one effect ────────────────────────────────
  useEffect(() => {
    if (stage !== "question") return;

    // ── Back-button block ────────────────────────────────────────────────────
    // Push a dummy entry so the back button pops it instead of leaving the page.
    ...
    // ── Multi-window detection via BroadcastChannel ──────────────────────────
    // If two windows have this interview open, one sends a ping, the other
    // responds — the responder gets flagged.
    const bc = new BroadcastChannel("smarthire_interview");
    bc.onmessage = (ev) => {
      if (ev.data === "ping" && qState === "revealed") {
        bc.postMessage("pong");
        recordViolation("Multiple browser windows detected");
      }
      if (ev.data === "pong") {
        recordViolation("Multiple browser windows detected");
      }
    };
    // Send a ping on mount — if another window is open it will pong us
    bc.postMessage("ping");
    ...
    // ── All below only active while question is revealed ─────────────────────
    ...
    const onPaste = (e) => {
      ...
      if (text.length > 200) {
        // Very long paste is suspicious — flag but don't forfeit, just warn
        clearTimeout(warnTimeRef.current);
    ...
    // ── DevTools detection (timing-based heuristic) ──────────────────────────
    // devtools open causes console.log with object to take noticeably longer
    let devtoolsOpen = false;
    const devtoolsCheck = setInterval(() => {
      const threshold = 160;
      const start = performance.now();
      // eslint-disable-next-line no-console
      console.log("%c", ""); // minimal side-effect log
    ...
    const onFullscreenChange = () => {
      if (!document.fullscreenElement) {
        recordViolation("Exited fullscreen view");
  ...
  // ── Timer: runs only after candidate reveals the question ───────────────────
  useEffect(() => {
  ...
  // ── Reveal question + enter fullscreen ──────────────────────────────────────
  const revealQuestion = () => {
    // Always fullscreen the root <html> element so the entire page — including
    // its background colour — is carried into fullscreen. Fullscreening a child
    // div paints everything outside that div black.
    if (document.documentElement.requestFullscreen) {
      document.documentElement.requestFullscreen().catch(() => {
        // Browser denied fullscreen (e.g. no prior user gesture in some builds).
        // Proceed anyway — the interview continues without fullscreen.
      });
    }
    setQState("revealed");
  };
  ...
  const submitAnswer = useCallback((marker) => {
    ...
    const isDisqualifiedMarker = marker === "__disqualified__";
    const a = isDisqualifiedMarker
      ? "[Disqualified — interview terminated]"
      : (current || "[No answer recorded]");
    ...
    // Save progress to localStorage in case of disconnect
    saveCheckpoint(newAnswers, qIdx + 1, violationsRef.current);

    if (isDisqualifiedMarker) {
      // Don't advance — disqualified stage will render instead
      finishInterview(newAnswers, true);
      return;
    }
  ...
  // Keep the ref in sync so the anti-cheat effect always has a fresh reference
  useEffect(() => {
    submitAnswerRef.current = submitAnswer;
    violationsRef.current = violations;
  }, [submitAnswer, violations]);

  const finishInterview = (allAnswers, wasDisqualified = false) => {
    if (wasDisqualified) {
      // Skip analysis — go straight to disqualified result
      const s = mockAnalyze();
      s.overall = 0; // zero out score
      s.disqualified = true;
      setScores(s);
      setStage("result");
      return;
    ...
    <div style={{ ...S.page, maxWidth: 720 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
        <div style={S.card}>
          ...
          <div style={{ background: "#000", borderRadius: 8, overflow: "hidden", aspectRatio: "4/3" }}>
        ...
        <div style={S.card}>
          ...
          {[
            ...
            "Type or speak your answer within 60 seconds.",
            "Exiting fullscreen, switching tabs, or minimizing the window counts as a violation.",
            "Repeated violations may disqualify your interview.",
          ].map((t, i) => (
      ...
      <div style={{ padding: "12px 16px", background: "#FEF3C7", border: "1px solid #FDE68A", borderRadius: 8, marginBottom: 20, fontSize: 13, color: "#92400E" }}>
        <strong>Integrity Notice:</strong> This interview is monitored. Exiting fullscreen, switching tabs, and minimizing the window are all logged as violations.
      </div>
  ...
  if (stage === "question") {
    ...
    return (
      ...
      <style>{`
        ...
        ::-webkit-backdrop { background:#f9fafb !important; }
      `}</style>
      <div style={{ ...S.page, maxWidth: 900, background:"#f9fafb", minHeight:"100vh" }} ref={pageRef}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            {/* Circular countdown — only visible once question is revealed */}
            {revealed && (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 16 }}>
          <div>
            {/* Question card — hidden or revealed */}
            <div style={{ background:"#ffffff", borderRadius:12, border:"1px solid #e5e7eb", padding:"1.25rem", marginBottom: 16, minHeight: 130 }}>
              ...
              {!revealed ? (
                ...
                <div style={{ textAlign: "center", padding: "16px 0" }}>
                  ...
                  <button
                    style={{ ...S.btnPrimary, fontSize: 14, padding: "10px 24px" }}
            ...
            {/* Answer box — only usable once revealed */}
            <div style={{ background:"#ffffff", borderRadius:12, border:"1px solid #e5e7eb", padding:"1.25rem", opacity: revealed ? 1 : 0.45, pointerEvents: revealed ? "auto" : "none" }}>
              <label style={{ fontSize:12, color:"#6b7280", fontWeight:600, marginBottom:4, display:"block" }}>Your Answer</label>
              <textarea
                style={{ width:"100%", padding:"9px 12px", borderRadius:8, border:"1px solid #d1d5db", background:"#f9fafb", fontSize:14, color:"#111827", fontFamily:"inherit", resize:"vertical", minHeight:120, boxSizing:"border-box", marginBottom:12 }}
                value={current}
                onChange={e => setCurrent(e.target.value)}
                placeholder={revealed ? "Type your answer here…" : "Reveal the question first to begin"}
                disabled={!revealed}
              />
              {revealed && (
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <button style={S.btnDanger} onClick={submitAnswer}>
                    Submit & Next →
                  </button>
          ...
          <div>
            <div style={{ background:"#ffffff", borderRadius:12, border:"1px solid #e5e7eb", padding:"1.25rem", marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
                <span style={S.badge("green")}>😊 Confident</span>
                <span style={S.badge("blue")}>👁️ Focused</span>
              </div>
            </div>
            ...
            <div style={{ background:"#ffffff", borderRadius:12, border:"1px solid #e5e7eb", padding:"1.25rem" }}>
              ...
              {questions.current.map((_, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, fontSize: 12 }}>
                  ...
                  <span style={{ color: i === qIdx ? "#111827" : "#9ca3af" }}>
                    Q{i + 1}
                  </span>
                </div>
              ))}
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center", maxWidth: 380 }}>
        ...
        <div style={{ fontSize: 13, color: "var(--color-text-secondary)" }}>
          {progress < 30 ? "Processing speech patterns…" : progress < 60 ? "Analysing facial expressions…" : progress < 85 ? "Scoring answer relevance…" : "Generating report…"}
        </div>
      </div>
    </div>
  );
  ...
  if (stage === "result" && scores) return (
    <div style={{ ...S.page, maxWidth: 680 }}>
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        ...
        <p style={S.muted}>{scores.disqualified ? "You were disqualified due to integrity violations." : "Here's your performance analysis"}</p>
        {violations > 0 && (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={{ fontWeight: 700, fontSize: 15, color: "#991B1B" }}>
                Disqualified — 3 Integrity Violations
              </div>
              <div style={{ fontSize: 13, color: "#991B1B", opacity: 0.8 }}>
...
export default function App() {
  ...
  // ── Camera: owned at the App level so it survives InterviewView re-renders ──
  // Start camera when entering interview view, stop it when leaving.
  // Using useEffect on `view` guarantees the stop runs BEFORE React unmounts
  // InterviewView, and the stream reference is never inside a child component.
  useEffect(() => {
    if (view === "interview") {
      ...
      // Stop all tracks the moment we leave the interview view
      setCameraStream(prev => {
        if (prev) prev.getTracks().forEach(t => t.stop());
        return null;
      });
    }
    // Stop on hard page close too
    const onUnload = () => {
      setCameraStream(prev => {
        if (prev) prev.getTracks().forEach(t => t.stop());
        return null;
  ...
  // On page load, check if there's a saved token and restore the session
  useEffect(() => {
    const token = localStorage.getItem("smarthire_token");
    if (token) {
      apiMe()
        .then(u => { setUser(u); setView(u.role==="hr" ? "hr-dash" : "candidate-dash"); })
        .catch(() => { clearToken(); })
  ...
  const login = (u) => { setUser(u); setView(u.role==="hr" ? "hr-dash" : "candidate-dash"); };
  const logout = () => { clearToken(); setUser(null); setView("landing"); };

  const Nav = () => (
    <nav style={S.nav}>
      <div style={S.navLogo} onClick={() => user ? setView(user.role==="hr"?"hr-dash":"candidate-dash") : setView("landing")}>
        <span style={S.navDot}/> SmartHire
      </div>
        ...
        {user ? (
          <>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              ...
              <span style={S.badge(user.role==="hr"?"blue":"green")}>{user.role==="hr"?"HR":"Candidate"}</span>
            </div>
  ...
  if (view==="interview") return (
    <div style={S.app}>
      <Nav/>
      <InterviewView
        job={interviewJob} application={interviewApp}
        stream={cameraStream}
  ...
  if (view==="hr-applicants" && viewingJob) return (
    <div style={S.app}>
      <Nav/>
      <ApplicantsView job={viewingJob} onBack={() => { setViewingJob(null); setView("hr-dash"); }}/>
    </div>
  );
